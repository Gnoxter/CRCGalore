...
crc_params = [
    ...
    # crc-catalog says init 0xff
    ["CRC-8/AUTOSAR", 0x12f, 0x00, False, 0xff, 0xdf],
    ...
    #crc-catalog: init 0x00 crcmod: init 0x55, same check
    ["CRC-8/ITU", 0x107, 0x55, False, 0x55, 0xa1],
    ...
    # crc-catalog says init 0xff
    ["CRC-8/SAE-J1850", 0x11d, 0x00, False, 0xff, 0x4b],
    ...
    #crc-catalog: init 0x0001 crcmod: init 0x0001, same check
    ["CRC-16/DECT-R", 0x10589, 0x0001, False, 0x0001, 0x007e],
    ["CRC-16/DECT-X", 0x10589, 0x00, False, 0x00, 0x007f],
    #crc-catalog: init 0x00 crcmod: init 0xffff, same check
    ["CRC-16/DNP", 0x13d65, 0xffff, True, 0xffff, 0xea82],
    #crc-catalog: init 0x00 crcmod: init 0xffff, same check
    ["CRC-16/EN-13757", 0x13d65, 0xffff, False, 0xffff, 0xc2b7],
    #crc-catalog says init 0xffff
    ["CRC-16/GENIBUS", 0x11021, 0x00, False, 0xffff, 0xd64e],
    ["CRC-16/LJ1200", 0x16f63, 0x00, False, 0x00, 0xbdf4],
    #crc-catalog: init 0x00 crcmod: init 0xffff, same check
    ["CRC-16/MAXIM", 0x18005, 0xffff, True, 0xffff, 0x44c2],
...
#   ["CRC-16/OPENSAFETY-A", 0x15935, 0x00, False, 0x00, 0x5d28],
    ...
    #crc-catalog says init 0xffff
    ["CRC-16/PROFIBUS", 0x11dcf, 0x00, False, 0xffff, 0xa819],
    #crc-catalog says init 0xb2aa, crcmod says 0x554d. same check
    ["CRC-16/RIELLO", 0x11021, 0x554d, True, 0x00, 0x63d0],
...
#	["CRC-24/BLE", 0x100065b, 0x555555, False, 0x00, 0xc25a56],
    ...
    #crc-catalog says init 0xffffff
    ["CRC-24/INTERLAKEN", 0x1328b63, 0x00, False,0xffffff, 0xb4f3e6],
    ["CRC-24/LTE-A", 0x1864cfb, 0x00, False, 0x00, 0xcde703],
    ["CRC-24/LTE-B", 0x1800063, 0x00, False, 0x00, 0x23ef52],

    #crc-catalog says init 0xffffffff
    ["CRC-32", 0x104c11db7, 0x00, True, 0xffffffff, 0xcbf43926],
    ...
    #crc-catalog says init 0xffffffff
    ["CRC-32/BZIP2", 0x104c11db7, 0x00, False, 0xffffffff, 0xfc891918],
    ["CRC-32/MPEG-2", 0x104c11db7, 0xffffffff, False, 0x00, 0x0376e6e7],
    #crc-catalog says init 0x0000000
    ["CRC-32/POSIX", 0x104c11db7, 0xffffffff, False, 0xffffffff, 0x765e7680],
    ...
    # crc-catalog says init 0xffffffffffffffff
    ["CRC-64/WE", 0x142f0e1eba9ea3693, 0x00, False, 0xffffffffffffffff, 0x62ec59e3f1a4f00a],
    # crc-catalog says init 0xffffffffffffffff
    ["CRC-64/XZ", 0x142f0e1eba9ea3693, 0x00, True, 0xffffffffffffffff, 0x995dc9bbdf1939fa],
...
parser = argparse.ArgumentParser(
    description='Calculates the CRC Sums for a given file. Useful to find the algorithm used for a given file and checksum without reverse engineering.')
parser.add_argument("-p", "--permutate",
                    help="Permutate to create non-standard configuration", action="store_true")
group = parser.add_mutually_exclusive_group()
group.add_argument(
    "-l", "--list", help="List available CRC Algorithms", action="store_true")
group.add_argument(
    "-c", "--code", help="Output C-Code for given CRC Algorithm", type=int)
group.add_argument(
    "-f", "--file", help="Calculate CRCs for given file", type=str)
...
if args.file:
    calculate_file(args.file)

if args.list:
    for index, crc in enumerate(crcs):
        print("{:3}: {:{spad}} {}".format(
            index, crc.name, crc, spad=max_length_name(crcs)))
